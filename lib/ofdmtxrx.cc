...
int defaultcallback(unsigned char *_header, int _header_valid,
		...
		framesyncstats_s _stats, void *_userdata)
{
	...
	if (_header_valid == 1)
	{
		if (_payload_valid == 1)
		{
			...
			if (_header_valid == NULL)
				header_valid = 0;
			int payload_valid = _payload_valid;
			if (_payload_valid == NULL)
				payload_valid = 0;
			int payload_len = _payload_len;
			if (_payload_len == NULL)
				payload_len = 0;
			try
			{
				py::bytes header((const char*) _header, 8);
				py::bytes payload((const char*) _payload, _payload_len);
				mycls->callback(header, header_valid, payload, payload_len,
						payload_valid, _stats.rssi, _stats.evm);
		...
		else
		{
			...
			try
			{
				...
				py::bytes header((const char*) _header, 8);
				py::bytes payload((const char*) _payload, _payload_len);
				mycls->callback(header, _header_valid, payload, 0,
						_payload_valid, _stats.rssi, _stats.evm);
			} catch (py::error_already_set &e)
			{
				//py::print("EXCEPTION defaultcallback");
	...
	else
	{
		...
		try
		{
			...
			py::bytes header((const char*) _header, 8);
			py::bytes payload((const char*) _payload, _payload_len);
			mycls->callback(header, _header_valid, payload, 0, _payload_valid,
...
ofdmtxrx::ofdmtxrx(const py::object &object, unsigned int _M,
		unsigned int _cp_len, unsigned int _taper_len, std::string _hint)
{
	...
	{
		fprintf(stderr,
				"error: ofdmtxrx::ofdmtxrx(), number of subcarriers must be at least 8\n");
		throw 0;
	}
	else if (_cp_len < 1)
	{
		fprintf(stderr,
				"error: ofdmtxrx::ofdmtxrx(), cyclic prefix length must be at least 1\n");
		throw 0;
	}
	else if (_taper_len > _cp_len)
	{
		fprintf(stderr,
				"error: ofdmtxrx::ofdmtxrx(), taper length cannot exceed cyclic prefix length\n");
		throw 0;
	}
	...
	fgbuffer = (std::complex<float>*) malloc(
			fgbuffer_len * sizeof(std::complex<float>));
	//fprintf(stderr,"error2\n");

	// create frame synchronizer
	fs = ofdmflexframesync_create(M, cp_len, taper_len, NULL, defaultcallback,
			this);
	// TODO: create buffer
	...
	fprintf(stderr, "Using %s with serial %s for TX/RX\n",
			dev_addrs.at(dev_id).get("name", "").c_str(),
			dev_addrs.at(dev_id).get("serial", "").c_str());
...
ofdmtxrx::ofdmtxrx(unsigned int _M, unsigned int _cp_len,
		...
		framesync_callback _callback, void *_userdata)
{
	...
	// create frame synchronizer
	fs = ofdmflexframesync_create(M, cp_len, taper_len, p, _callback,
			_userdata);
	// TODO: create buffer
...
ofdmtxrx::ofdmtxrx(unsigned int _M, unsigned int _cp_len,
		unsigned int _taper_len, unsigned char *_p,
		framesync_callback _callback, void *_userdata, bool _blocking_rx_worker)
{
	// validate input
	if (_M < 8)
	{
	...
	{
		pthread_create(&rx_process, NULL, ofdmtxrx_rx_worker_blocking,
				(void*) this);
	}
	else
	{
...
void ofdmtxrx::transmit_packet_python(std::string _headerstr,
		std::string _payloadstr, unsigned int _payload_len, int _mod, int _fec0,
		int _fec1)
{
	...
	this->transmit_packet((unsigned char*) _header, (unsigned char*) _payload,
			_payload_len, _mod, _fec0, _fec1);
	//fprintf(stderr,"transmit_packet_python %s\n", _header);
}

// update payload data on a particular channel
void ofdmtxrx::transmit_packet(unsigned char *_header, unsigned char *_payload,
		unsigned int _payload_len, int _mod, int _fec0, int _fec1)
{
	...
	while (!last_symbol)
	{
		...
		tx_stream->send(&usrp_buffer.front(), usrp_buffer.size(), metadata_tx);
...
void ofdmtxrx::assemble_frame(unsigned char *_header, unsigned char *_payload,
		unsigned int _payload_len, int _mod, int _fec0, int _fec1)
{
	// set properties
	fgprops.mod_scheme = _mod;
...
void ofdmtxrx::set_timespec(struct timespec *_ts, float _timeout)
{
...
void* ofdmtxrx_rx_worker(void *_arg)
{
	...
	while (txcvr->rx_thread_running)
	{
		...
		while (txcvr->rx_running)
		{
			...
			if (txcvr->usrp != NULL)
			{
				num_rx_samps = txcvr->rx_stream->recv(&buffer.front(),
						buffer.size(), md);
...
void* ofdmtxrx_rx_worker_blocking(void *_arg)
{
	...
	while (txcvr->rx_thread_running)
	{
		...
		while (txcvr->rx_running)
		{
			...
			pthread_cond_wait(&txcvr->rx_buffer_modified_cond,
					&txcvr->rx_buffer_mutex);
			// push data through frame synchronizer
			// TODO : use arbitrary resampler?
			unsigned int j;
			for (j = 0; j < num_rx_samps; j++)
